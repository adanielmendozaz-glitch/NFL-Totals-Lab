...
fun ProgressiveCalibrationScreen(state:CalibrationState,snapshots:List<CalibrationSnapshot>,onBack:()->Unit){
    ...
    Column(Modifier.fillMaxSize()){
        Row(Modifier.fillMaxWidth().padding(14.dp),verticalAlignment=Alignment.CenterVertically){OutlinedButton(onClick=onBack){Text("←")};Column(Modifier.padding(start=10.dp)){Text("PROGRESSIVE CALIBRATION",fontWeight=FontWeight.Black,fontSize=18.sp);Text("Rolling pregame · freeze al kickoff · fuera de muestra",fontSize=10.sp,color=MaterialTheme.colorScheme.onSurfaceVariant)}}
        LazyColumn(Modifier.fillMaxSize().padding(horizontal=14.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
            ...
            items(snapshots.sortedByDescending{it.createdAt}.take(80)){x->Surface(shape=RoundedCornerShape(13.dp),border=BorderStroke(1.dp,MaterialTheme.colorScheme.outlineVariant)){Row(Modifier.fillMaxWidth().padding(11.dp),verticalAlignment=Alignment.CenterVertically){Column(Modifier.weight(1f)){Text("${x.awayTeam} @ ${x.homeTeam}",fontWeight=FontWeight.Black,fontSize=11.sp);Text("W${x.week} · train ${x.trainN} · ${x.maturity} · ${x.result?:"PREGAME"}",fontSize=9.sp,color=MaterialTheme.colorScheme.onSurfaceVariant)};Column(horizontalAlignment=Alignment.End){Text("RAW ${(x.rawProbability*100).f1()}%",fontSize=10.sp);Text("CAL ${(x.calibratedProbability*100).f1()}%",fontWeight=FontWeight.Black,fontSize=11.sp,color=MaterialTheme.colorScheme.primary)}}}}
            item{Text("CAL se actualiza solo con FINALs anteriores mientras siga pregame. Al kickoff queda congelado. MEJORA > 0 = menor Brier.",fontSize=9.sp,color=MaterialTheme.colorScheme.onSurfaceVariant,modifier=Modifier.padding(vertical=8.dp))}
        }
    }
}
